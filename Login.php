...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    $stmt = $conn->prepare("SELECT * FROM registeredacc WHERE username = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($inputPassword, $user['password'])) {
            ...
            echo "<script>
                    Swal.fire({
                        ...
                        window.location.href = '" . ($role === 'Admin' ? 'Admin_page.php' : 'volunteer.php') . "';
                    });
                  </script>";
            exit();
        } else {
        ...
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Login Failed',
                    text: 'Invalid username or password 123.',
                    confirmButtonColor: '#3085d6'
                }).then(() => {
                    window.location.href = 'login.php';
                });
